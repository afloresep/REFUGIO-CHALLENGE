...
import numpy as np
from warehouse_api import Action, CellType
...
def create_layout():
    ...
    if extra > 0:
        if REMOVAL == "spread":
            ...
            def ad(c):
                return sum(abs(c[0] - ex) + abs(c[1] - ey) for ex, ey in ent)

            cells.sort(key=lambda c: (ad(c), c[1], c[0]))
            cells = cells[:960]
        else:  # central
            cells.sort(key=lambda c: (abs(c[0] - 25.5) + abs(c[1] - 25.5), c[1], c[0]))
            cells = cells[:960]
